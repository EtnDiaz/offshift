...
| `preview_dashboard_transition` | the widget's short-lived public-demo session expired while recording | action and optional duration | presentation-only dashboard state | read-only, app-only; never schedules a plan or invokes Companion, device, scene, lock, webhook, or external action |
...
On the public Worker host, `render_offshift_dashboard` returns the final snapshot, behaviour explanation, and plan and attaches the versioned `ui://widget/offshift-worker-v7.html` resource. It is the only model-visible tool that mounts the widget and mints a five-minute dashboard capability in result `_meta`, never in model-visible content. Component-initiated mutations use `tools/call` through the MCP Apps bridge and must present that server-verified capability. If that deliberately non-durable public-demo session is lost between render and click, the widget calls the app-only, read-only `preview_dashboard_transition` fallback and labels the resulting state as a demo preview; it cannot make any real change. The resource shell explicitly loads its compiled stylesheet from the CSP-allowlisted Worker asset origin. The separate local Node demo has its own versioned resource URI and is not the public deployment.
