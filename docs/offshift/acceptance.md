...
| Worker MCP HTTPS | Public deployment and MCP host contract | Pass (smoke renderer) | `https://offshift-demo-api.tixo-digital.workers.dev/mcp` returned `initialize`, all six tools, `ui://widget/offshift-worker-v1.html`, and `text/html;profile=mcp-app` over HTTPS on 2026-07-16. The Worker widget is intentionally a bridge smoke renderer, not the React dashboard. |
