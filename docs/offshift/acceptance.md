...
| Worker MCP HTTPS | Public deployment, React widget resource, and mutation authority contract | Pass (contract) | `https://offshift-demo-api.tixo-digital.workers.dev/mcp` returned health with `ui://widget/offshift-worker-v3.html`; `tools/list` exposed eight tools, with every mutation visible only to `app`; `resources/read` returned `text/html;profile=mcp-app` and the Worker-hosted React bundle returned HTTP 200, 2026-07-16. ChatGPT-host rendering remains unverified. |
...
| macOS companion | Multi-factor policy, local aggregate sampler, night-care overlay, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 22 tests; quiet-hours boundary tests cover an overnight 23:00–07:00 schedule, and the existing tests prove gate and lock-rule boundaries. `./script/build_and_run.sh --verify` plus Computer verification enabled Night care locally, triggered a late-session fixture, and confirmed an automatic, readable protection window at 00:29 with clear reason, `Take 5`, bounded on-call, and pause-until-tomorrow controls. No automated or UI test invokes the system Lock Screen. |
