...
| Care preview | `./script/build_and_run.sh --care-preview` launches the local visual-only process with `--care-preview`, normal activation, and a floating black QA window. Real local behaviour remains screen-saver-level. The preview cannot enter a Lock Screen countdown or execute Home Assistant; those boundaries are covered by focused tests. |
...
2. Computer Use cannot obtain accessibility state for the current Debug care preview either, even after it is made a normal-activation floating window; it lists the running app but `get_app_state` times out. The four-Escape gate is unit-tested, but a human desktop pass is still required before claiming a visual/live-keyboard result.
