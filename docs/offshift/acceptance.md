...
| macOS companion | Multi-factor policy, local aggregate sampler, black monitor-covering intervention, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 27 tests; overnight quiet-hours and pause-end boundaries, bounded on-call expiry, the three-cancellation fail-closed Lock consent gate, and one-per-drift gentle-night-nudge policy are covered. `./script/build_and_run.sh --verify` plus Computer verification at 01:18–01:19 confirmed that both `Protect` and `Gentle night nudge` open a borderless black monitor-covering surface. Every affirmative exit (`Take 5`, pause, successful on-call, or `Turn Offshift off`) explicitly dismisses it; `Take 5` also resets the aggregate activity window, so the current Drift episode cannot re-open it. The new local `Sleep care` → `Off` transition cancels sampling, countdowns, and interventions and is the only transition a future, separately entitled Screen Time adapter may invoke; it does not read Apple Screen Time in the MVP. The gentle path has no on-call or Lock Screen controls. `Protect` retains on-call/cancel controls and, only with its separately enabled local rule, uses a visible 10-second countdown before the macOS Lock Screen request. No automated or UI test invokes the system Lock Screen. |
