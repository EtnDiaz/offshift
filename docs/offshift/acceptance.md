...
| macOS companion | Multi-factor policy, local aggregate sampler, black monitor-covering intervention, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 27 tests; overnight quiet-hours and pause-end boundaries, bounded on-call expiry, the three-cancellation fail-closed Lock consent gate, and one-per-drift gentle-night-nudge policy are covered. `./script/build_and_run.sh --verify` plus Computer verification at 01:18–01:19 confirmed that both `Protect` and `Gentle night nudge` open a borderless black monitor-covering surface. The gentle path showed its aggregate reason, `Take 5`, and `Pause until 7 AM`; on-call and Lock Screen controls were absent and no countdown ran. `Protect` retains on-call/cancel controls and, only with its separately enabled local rule, uses a visible 10-second countdown before the macOS Lock Screen request. No automated or UI test invokes the system Lock Screen. |
