...
| Widget | Type safety, bundle, safe loading/error copy, explicit reversible controls | Pass locally | `pnpm run tsc:app`; `pnpm run build --target offshift`; widget resource bumped to `ui://widget/offshift-v4.html`, 2026-07-16. Full ChatGPT host rendering remains unverified. |
...
| Worker MCP HTTPS | Public deployment, React widget resource, and mutation authority contract | Pass (contract) | `https://offshift-demo-api.tixo-digital.workers.dev/mcp` returned health with `ui://widget/offshift-worker-v4.html`; `tools/list` exposed eight tools, and every mutation is `app`-visible plus requires a server-verified five-minute dashboard capability. A public round-trip verified v4 `resources/read` with `text/html;profile=mcp-app`, a missing capability rejection, and a valid metadata-only capability that was absent from `structuredContent` and could only prepare a bounded demo plan, 2026-07-17. ChatGPT-host rendering remains unverified. |
| ChatGPT | Developer Mode connection and golden prompts | Blocked by ChatGPT UI entitlement | Developer Mode is enabled in a signed-in ChatGPT session, and the official `chatgpt.com/plugins` route was opened after the setting change. The rendered page exposes the plugin catalog but no plus/create control (and no accessible create button), so the approved Worker cannot yet be added or exercised by the ChatGPT host. This is a product-UI/entitlement mismatch, not a pass or a server failure; external Worker health and `tools/list` remain verified, 2026-07-17. |
| macOS companion | Multi-factor policy, local aggregate sampler, night-care overlay, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 26 tests; overnight quiet-hours and pause-end boundaries, bounded on-call expiry, and the three-cancellation fail-closed Lock consent gate are covered. `./script/build_and_run.sh --verify` plus Computer verification enabled Night care locally, confirmed the early-start setting, and triggered an automatic, scrollable protection window at 00:41 with clear reason, `Take 5`, bounded on-call, and an exact quiet-hours pause end. No automated or UI test invokes the system Lock Screen. |
