...
| macOS companion | Multi-factor policy, local aggregate sampler, animated original care mascot, black monitor-covering intervention, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 27 tests; `./script/build_and_run.sh --verify`; Computer verification at 10:20–10:22 showed the dashboard’s primary next step and a borderless black care surface containing the original sleeping Nox mascot and animated blue z marks. Its default keyboard action reads “Start a 5-minute break and leave”; helper text stays high-contrast. A quiet-hours fixture reads “A kind time to call it tonight” and explains that work/tokens remain. The local `Sleep care` → `Off` transition cancels sampling, countdowns, and interventions; it does not read Apple Screen Time in the MVP. The gentle path has no on-call or Lock Screen controls. `Protect` retains on-call/cancel controls and, only with its separately enabled local rule, uses a visible 10-second countdown before the macOS Lock Screen request. No automated or UI test invokes the system Lock Screen. |
