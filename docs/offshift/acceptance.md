...
| Worker MCP HTTPS | Public deployment, React widget resource, and mutation authority contract | Pass (contract) | `https://offshift-demo-api.tixo-digital.workers.dev/mcp` returned health with `ui://widget/offshift-worker-v3.html`; `tools/list` exposed seven tools with all four mutations visible only to `app`; `resources/read` returned `text/html;profile=mcp-app` and the Worker-hosted React bundle returned HTTP 200, 2026-07-16. ChatGPT-host rendering remains unverified. |
...
| macOS companion | Multi-factor policy, local aggregate sampler, shadow mode, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 20 tests; three tests prove the local gate suppresses interventions until its expiry, rejects a past deadline, and remains off until explicit local enable. A one-minute sampler produces opaque aggregate intervals only; quiet hours + two snoozes can escalate a sustained session, while no activity cannot; default configuration suppresses a fired pre-lock countdown before any adapter call; enabled rules have a one-attempt-per-protect-episode limit. `./script/build_and_run.sh --verify` plus Computer UI smoke-tests verified visible Pause → Resume and Turn off → Turn on flows, restored to Active afterwards. No automated test invokes the system Lock Screen. |
