...
| macOS companion | Multi-factor policy, local aggregate sampler, night-care overlay, reversible pause/off gate, local host, one direct-confirmation scene, and optional system Lock Screen rule | Pass (non-locking tests) | `swift test` — 27 tests; overnight quiet-hours and pause-end boundaries, bounded on-call expiry, the three-cancellation fail-closed Lock consent gate, and one-per-drift gentle-night-nudge policy are covered. `./script/build_and_run.sh --verify` plus Computer verification enabled Night care locally, confirmed the early-start setting, and at 01:13 triggered the `Gentle night nudge` path: the care window showed its aggregate reason, `Take 5`, and `Pause until 7 AM`; on-call and Lock Screen controls were absent and no countdown ran. No automated or UI test invokes the system Lock Screen. |
