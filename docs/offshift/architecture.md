...
`offshift_worker` is the public HTTPS MCP **demo** host at `https://offshift-demo-api.tixo-digital.workers.dev/mcp`. It serves the versioned `ui://widget/offshift-worker-v6.html` React resource, with its compiled CSS loaded from the Worker asset origin explicitly allowlisted by widget CSP. Rendering mints a short-lived, opaque dashboard capability in result metadata; the Worker validates it before each bounded plan mutation and keeps plan/idempotency state scoped to that in-memory capability session. This is intentionally non-durable fixture data, not user identity, tenant isolation, or device authority. It must not be used for real personal status or schedules. A public multi-user service requires OAuth 2.1, server-derived account subjects, tenant-scoped durable storage, and explicit companion pairing under ADR 0021.
